...
class ComparisonBenchmark {
  ...
  private async benchmarkHuffman(tokenCount: number, iterations: number): Promise<number[]> {
    const times: number[] = [];
  ...
  private async benchmarkSSE(tokenCount: number, iterations: number): Promise<number[]> {
    const times: number[] = [];
